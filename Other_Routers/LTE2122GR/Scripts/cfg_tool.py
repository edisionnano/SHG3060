...
PRIVATE_KEY = bytes.fromhex("4133353636393730304634374641333530424536303444423442423043333145")
PRIVATE_IV = bytes.fromhex("34423733373839343937313341433635")
PASSWORD = "abcdefgh"
...
def derive_cfg_key(password_str):
    ...
    return hashlib.md5(handle32).digest() + hashlib.md5(PRIVATE_KEY).digest()
...
def decode_confxml(blob):
    return blob.split(b'\x00')[0]


def encode_confxml(xml_bytes):
    return xml_bytes + b'\x00' + util_crc32(xml_bytes)
...
def cmd_decode(cfg_path):
    ...
    cfg_key = derive_cfg_key(PASSWORD)
...
def cmd_encode(cfg_path):
    ...
    encrypted_content = cfg_encrypt_payload(compressed, len(payload), cfg_key)
    b64_body = pkcs7_sign_content(encrypted_content)

    final = build_cfg_text_header(headers) + b64_body + b'\n'
    (out_dir / 'Config3UKBox_new.cfg').write_bytes(final)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('command', choices=['decode', 'encode'])
    parser.add_argument('--cfg', default='Config3UKBox.cfg')
